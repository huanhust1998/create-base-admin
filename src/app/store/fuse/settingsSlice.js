...
export function generateSettings(_defaultSettings, _newSettings) {
    const response = _.merge(
        {},
        _defaultSettings,
        {layout: {config: FuseSettingsConfig[_newSettings?.layout?.style]?.defaults}},
        _newSettings
    );
    ...
    Object.entries(response.theme).forEach(([key, value]) => {
        if (value !== "mainThemeDark" && value !== "mainThemeLight" && !FuseSettingsConfig[value]) {
...
export const selectFuseThemeByIs = (id) =>
    createSelector([getThemes, getDirection], (themes, direction) =>
        generateSettings(themes, id, direction)
    );

export const selectContrastMainTheme = (bgColor) => {
    function isDark(color) {
        return getContrastRatio(color, "#fffff") >= 3;
    }
    ...
    reducers: {
        ...
        setDefaultSettings: (state, action) => {
            ...
            const themes =
                defaults.theme.main !== state.defaults.theme.main
                    ? {...state.theme, ...mainThemeVariations(themesObjRaw[defaults.theme])}
                    : state.themes;
        ...
        resetSettings: (state, action) => {
            ...
            return {
                ...
                currents: _.merge({}, state.defaults),
                themes,
            };
        },
    },
